...
- **#418 (0.9.148) — multi-site customer delivery-address resolution.** New _by_delivery_address() rung in customer.resolve(): when 2+ customer cards share a sender email and _by_store() fails, matches delivery city/street from the raw email text against card fields. One hit = that card (rule=delivery_address); zero/multiple = customer board question. pipeline.py threads delivery_text=free_text. No ORDER_SCHEMA change, no corpus re-record. Tests: 8 new cases (RED 72c2501 -> GREEN 0b4c81b). /znalosti customer form already had city/street fields.
- **#420 (0.9.149) — hold + ask a typo-corrected delivery date instead of dropping it.** A written day.month that conflicts with the model's corrected date (body "14.8." + "pondelok" → model 14.09.2026, a Monday) tripped #163's grounding poistka, which silently DROPPED the whole order → "(nezistený zákazník)" dead end (0 attachments = no board question after #404). 13× in 60 days. Fix: `extract.date_ground_conflict()` returns the written day(s) the extracted date matches none of (`date_grounded()` is now a thin bool wrapper — semantics unchanged); `extract.run()` KEEPS the order and sets `result["date_conflict"]` with candidate days (next-future written d.m via `_next_future_day_month`, model date, mail+7) + a Slovak reason instead of the "…sa nenašiel" note; `pipeline._run()` folds it into the SAME existing hold + `teach.ask_date` path (customer + items resolve, order HELD, `date` board question, summary shows the conflict). Shadow stays review. db_schema: +1 match_incidents row. Corpus (dev2): +1 case zakaznik-s-2026-09-11-datetypo (anonymized from prod msg 11059), llm-cache + baseline updated, 37-case gate green; ORDER_SCHEMA + prompts/extract.md unchanged → existing cache valid. Fable review PASS (0🔴, 1🟡+4🔵 fixed in 0945e4e: candidates chronological + capped + junk-filtered, gc-reason precedence, ISO-prefix parse). Tests: RED cf0e1d6 → GREEN a1961ad. Merged via PR #422, deployed 0.9.149 (/health ok, startup clean).
- **#421 (0.9.150) — nástenka „Vyriešené ručne" 409 pre osirelé položkové otázky + hold zaseknutý na expirovanej otázke.** Tri opravy: (1) `pipeline.py:470` položková ask-brána teraz vylučuje `is_change` (parita s :522/:559) — change-request mail už neklade osirelé item otázky. (2) `_api_orders_answer_item_manual` rozlišuje „nikdy nedržané" (zavri ako `manual`, 200, `answered_by=session role`) od „released/deadline" (presné 409); Fable F1: refuse 409 aj keď mail už čiastočne odišiel do ORIONu (uploaded_orion event nesie qid) — nikdy duplicitná dodávka; F2 post-answer TOCTOU re-check. (3) `hold.close_expired_holds` — STATE-based sken (held riadok, žiadna open otázka, ≥1 expired), beží každý tick z `expire_stale`, zavrie bez shipu (`release_reason='expired'`) + ops alert; pokrýva aj staggered dedup (F4) bez zásahu do `_release_locked`; F3 guarded expiry UPDATE. db revízia 14: rozšíri release_reason CHECK o 'expired' + one-off backfill 41/42 (F7 guard `question_ids && ARRAY[151,154]`, nikdy re-ship). RED tests naprieč test_orders_pipeline/test_httpapi_manual_421/test_question_alerts/test_db (RED 981777e → GREEN 5049c9b → review-fixes f248bb3). Fable review (gate OPEN): 1🔴 4🟡 2🔵, všetko okrem F8 (size refactor, deferred) opravené v tej istej vetve. Lokálne: full suite green 93.66% cov, ruff+mypy clean. Playbook: schema-migrations.md (CHECK-widening + reapply dirty-container artefakt, `::bigint[]` overlap cast), git-commit-hygiene.md (Shared-benefit: line gate #877).
- **#426 (0.9.151) — „➕ Nová karta" priamo na položkovej otázke nástenky objednávok.** Posledná „nová vec rovno na otázke" akcia, ktorá chýbala (zákazník ju má cez new_customer #234, obe DL cez new_supplier/new_item #235). Server: `_api_orders_answer_new_product` dispatchnutý v `api_orders_answer` pre `kind=='item'` PRED kontrolou existujúcej karty — validácia čísla položky (digits) + názvu (+ voliteľný doplnok, tri-state ako `/api/znalosti/products`), 409+`existing` pri kolízii so živou kartou (`catalog_for_management`, kryje base-snapshot aj override, vylučuje retired), inak v JEDNEJ `db_tx`: `upsert_catalog_card`+`rebuild_from_overrides`+`add_candidate({gtin,name})`+`teach.answer(by="sklad-new-card", quantity, unit_price)`, potom `hold.release_for_question` na SEPARÁTNOM autocommit spojení (answer-commits-before-upload split z PR 116). UI: inline item vetva v `load()` extrahovaná do `itemQuestionCard(q)` + nový `newProductForm(q)` (tlačidlo „➕ Nová karta (číslo položky z CODEXu)", polia číslo položky*/názov*/doplnok, `form.dataset.open` proti 5 s refreshu, POST `{new_product:{gtin,name,doplnok?}, quantity, unit_price}` nesúci lineFields hodnoty; 409 → tlačidlo „Použiť existujúcu kartu" cez `teach()`). Hint „pridaj v databáze znalostí" teraz ukazuje na tlačidlo, `/znalosti` link ostáva sekundárny. Bez zmeny security (`SKLAD_ACTION` už kryje endpoint, `ORDERS_KINDS` obsahuje `item`); zápis do `catalog_overrides` presne ako cez `/znalosti`. Template hashe re-pinnuté (ASK_HTML + ASK_DL_HTML, obe zdieľajú `_ASK_HTML_TEMPLATE`). Fresh-context review (7 lenses): 0🔴 0🟡 1🔵 — 🔵 je PRE-EXISTING nechránený `teach.answer` UPDATE (bez `WHERE status='open'`, identické s bežnou item vetvou), backstopnutý downstream `_release_locked` FOR UPDATE + `edi_sent` ledgerom → dropnuté in-lane, followup pre supervízora. RED tests (`test_httpapi_new_product.py`) → GREEN; ruff+mypy clean, cielené suity green.
- **#428 (0.9.152) — teach.answer atomický súbeh-guard.** Posledná question-settle cesta bez atomického guardu (nález z #426 review, 🔵 pre-existing). `teach.answer` (item / #426 new_product / #360 line edit) robila check-then-act: Python `status != 'open'` + `UPDATE … WHERE id=%s` bez `AND status='open'`/`RETURNING`, pamäťové zápisy PRED UPDATE-om → dve súbežné odpovede sa obe „ujali" + dva `item_memory` zápisy (duplicitný ship nehrozil vďaka `_release_locked` FOR UPDATE + `edi_sent`). Fix zrkadlí `answer_customer` (#234) + `_api_orders_answer_generic` (#323): atomický `WHERE id=%s AND status='open' RETURNING id`, na 0 riadkov `raise AlreadyAnswered` (→ endpoint 409) bez efektu, `memory.remember*` presunuté AŽ ZA guard. Volajúci nezmenení (item tail + new_product už chytajú `AlreadyAnswered→409`; new_product `db_tx` rollbackne aj zápis karty; `_apply_item` dedí). RED `run_racers` (dve reálne vlákna, rôzne gtiny → na starom kóde obe vyhrajú + 2 human `item_memory` riadky) `287f2aa` → GREEN `15ba97a`. Adversárny fresh-context review (7 lenses): 0🔴 0🟡 0🔵. Celá suita green. Merged PR #429 (`f662f637`), nasadené 0.9.152 (/health 200 v0.9.152, štart čistý, guard v bežiacom obraze `teach.py:201`). Playbook: board-line-edit.md (question-settle concurrency-guard doktrína).
- **#431 (0.9.154) — pád pipeline (hold.place matched=None) → tichá strata objednávky.** Change-request mail od odosielateľa s 2 kartami zákazníka (`customer.resolve()=None`, #418) + rozpor dátumu (#420) volal `hold.place(matched=None)` (placeholder `hold_matched` bol vnorený pod `if matched is None and not is_change:`, takže change-request ho preskočil) → `AttributeError 'NoneType'…'ean_edi'` na `hold.py:119`, celá objednávka spadla bez otázky = tichá strata (PROD 2026-09-14 12:07, gazdovskytrh „GT1 cyklická"). Oprava: (1) `pipeline.py` placeholder `hold_matched` sa priradí VŽDY keď `matched is None` (`if matched is None:` vonku, `customer` otázka v inom `if not is_change:` — change-request drží na `date` otázke bez crashu, per #421); (2) `hold.place` odmietne `matched=None` `ValueError`-om (choke-point pre všetky 3 call-sity); (3) `worker.tick` + `static_worker.tick` (parita #372/#373) na finálnom páde enqueue durable ops-alert (`pending_alerts`, vlastný kind per engine, `crash_outcome` HTML-escapnuté). Reprodukované na dev2 (is_change=True padá, is_change=False už OK). Fresh-context adversárny review (7 lenses): 0🔴, F1 static parita/F2 escape/F4 test opravené in-lane, F3 (`status='error'`) zdôvodnené. RED (`287f2aa`… v test_orders_pipeline/hold/worker) → GREEN; ruff+mypy clean, full suite 93.65% cov, e2e-orders+e2e-dl korpus green. PR #432 merged (`1892ef9`), nasadené 0.9.154 (/health v0.9.154, DOM v0.9.154, štart čistý, console 0). Živá náprava: spadnutá správa 11404 pustená sankcionovaným resetom (+`attempts=0`, keďže bola na MAX_ATTEMPTS) → reprocess pod 0.9.154 bez crashu, 5 objednávok (4 held + 1 same-day review) na nástenke + Odoo ch.152 „⌛ čaká na sklad", nič do ORIONu, `date` otázka neodpovedaná. Playbook: `.claude/rules/hold-crash-safety.md` (nový — hold.place matched=None invariant, crash→ops-alert parita, attempts=0 reset).
- **#424 (0.9.155) — čistý refaktor: rozdelenie hold.py + extrakcia _classify_manual_target (ZERO zmena správania).** Nález z Fable #421 review (F8). Dve časti, obe byte-exact: (1) `app/orders/hold.py` (930 r.) → tenká FACADE (67 r.) re-exportujúca KAŽDÝ verejný + testom-siahaný privátny názov, nad tromi concern modulmi podľa zodpovednosti (module-split-refactor.md #309 vzor): `hold_place.py` (leaf: deadline, de/serializácia rozhodnutí, place/get/list), `hold_redecide.py` (re-decision + ship helpery; importuje hold_place), `hold_close.py` (release/expire/manual-resolve/customer+date/deadline-sweep; importuje hold_place+hold_redecide). Presun poháňaný scratch AST skriptom (mimo git): contiguous chunk `lines[prev_end:end_lineno]`, auto-počítané importy cez tokenize, verifikácia `ast.get_source_segment` rovnosti (31/31 identických), per-modul `log = getLogger("orders.hold")` verbatim, acyklický DAG. (2) `httpapi_orders_questions._api_orders_answer_item_manual` (~170 r.): READ-ONLY verdikt vyextrahovaný do modul-level `_classify_manual_target(conn, qid)` (noop/never_held/already_shipped/released_*/multi_message/held); endpoint je teraz tenký dispatcher — HTTP kódy + správy byte-for-byte, zápisy + post-answer TOCTOU re-checky ostávajú v endpointe (klasifikátor je čisto čítací). #431 hold-crash-safety (place odmieta matched=None) + #360/#428 board-line-edit invarianty (_apply_confirmed_quantities, atomický `WHERE status='open' RETURNING`) sa presunuli nezmenené. Charakterizačný test (`test_hold_split_characterization.py`) pinuje verejné API hold (názvy+signatúry+log-name+_COLS) + manual-endpoint odpovede pre každý verdikt (byte-exact status+správa) — commitnutý PRED presunom, green pred aj po, dokázateľne padá na drop/drift. Fresh-context adversárny review (7 lenses): 0 behaviour drift; 1🔴 (ruff E702 v novom teste, `c = _client(); _login(c)` — CI beží `ruff check .`) opravený in-lane (`7681060`, per #311). Lokálne: full suite 2041 passed / 0 fail / 0 skip, ruff clean, mypy clean (84 files), e2e korpus (replay z cache, žiadny schema-change) green. Playbook: bez nového pravidla (module-split-refactor.md + git-commit-hygiene.md už pokrývajú vzor + design-gate Shared-benefit).
- **#435 (0.9.156) — Košík.sk prevádzky: rodina kariet rozhoduje podľa adresy doručenia NAD llm pickom.** #418 dokončenie: `customer.resolve` vracal istý llm-pick centrály (Košice 2000000000797) PRED akoukoľvek adresnou logikou, a „rodinu" detegoval len podľa zhodného plného e-mailu odosielateľa — `supply@kosik.sk` nebol v žiadnej karte, takže 6 objednávok (msg 11298–11303) išlo `customer_rule=llm` → Košice, jedna reálne pre MAKRO Žilina. Oprava: vo viac-prevádzkovej RODINE rozhoduje adresa doručenia (mesto/ulica) NAD llm pickom. Rodina = **A** `owners>1` (zhodný plný e-mail) **alebo B** (odosielateľ v žiadnej karte + istý model-pick) karty so zhodným rozlišujúcim kmeňom názvu picku (`_name_stem`: `dl_match.fold` + prvý ne-generický ne-číselný token; grupuje „Košík.sk …" ale drží dve rôzne firmy so zdieľaným generickým prefixom „Centrum pre deti a rodiny …" oddelene). `_by_store`→`_by_delivery_address` vyberie práve jednu → tá karta (`delivery_address`/`store_address`); bez jednoznačnej zhody → None → nástenková `customer` otázka, NIKDY nie centrála. Odosielateľ jednoznačne vlastniaci 1 kartu (`owners==1`) je z kmeň-cesty vylúčený → nezmenená llm/exact_email hierarchia. **Korpus-safe (dokázané):** kmeň-cesta beží LEN pri `owners==0`; všetkých 6 korpus `should_ship` case-ov s `owners!=1` má rodinu veľkosti 1 (kosik/agel/domovina/kolinovce unikátne v `customers.csv`); offline `--require-all` 32/37 (5 = KNOWN DEFECT #120), oba Košík case-y → 797 nezmenené. Zamietnutá alternatíva: rodina podľa e-mailovej DOMÉNY (rozbila by PNO reťazec zdieľajúci `potravinynieotraviny.sk`, `owners==1`). RED (5 nových #435 testov + 2 prepísané #418 testy s odôvodnením) `c33b6bb` → GREEN `9b3d2ba`; review-nález 🔵 (brand-token kolízia, safe-by-default → otázka) zafixovaný testom `2b912e4`. Fresh-context adversárny review (7 lenses): 0🔴 0🟡 1🔵 (adresovaný). Full suite 2048 passed / 0 fail / 0 skip, ruff+mypy clean. PR #438 merged (`6b8bd44`), nasadené 0.9.156 (/health v0.9.156, štart čistý). Živé SHADOW overenie (read-only, žiadne zápisy): snapshot 30 už má 3 Košík karty (871 Žilina/872 Zvolen/797 Košice, všetky supply@kosik.sk); reálne správy → msg 11303 (17.9. „Prielohy 1, Žilina") → **871 Žilina** `delivery_address`, msg 10948 (14.9. 4500317337 „Zvolen") → **872 Zvolen**, msg 10947 (14.9. 4500317338 „Žilina") → **871 Žilina**. Playbook: `.claude/rules/customer-resolution.md` (nový — rodina kariet, kmeň názvu, korpus-safety brána owners==0).
- **#436 (0.9.157) — sken z tlačiarne s „úspešným" OCR (CMR/iný papier) nedostal vision + alert išiel len na ops 592.** Dve root cause (msg 11503: HK Loan CMR skenovaný tlačiarňou, OCR 3609 zn., `needs_vision=false`): (1) `human_processing._rescue` brána bola `needs_vision AND has_attachments` — `needs_vision` meria „OCR ~prázdne", takže scan s použiteľne DLHÝM ale pre DL NEPOUŽITEĽNÝM textom (CMR/výplatná páska) vision (druhý názor nad OBRÁZKOM) nikdy nedostal; rovnaká brána zablokovala 17/24 skenov z tlačiarne. (2) `_notify` posielal každý alert na ops (592), aj pre skener (`delivery_notes_scanner_senders` = vždy skladový papier), takže sklad (sleduje 243) sa nedozvedel, že jeho sken nebol DL. Oprava: brána = `has_attachments AND _ocr_unusable_for_dl(combined_machine_text)` (silné signály „použiteľné"=preskočiť vision: EAN-13 / ≥2 množstvo+jednotka riadky / SK DL-kľúčové slovo `dodac…list`/`objednávka`/`reklamácia`/`faktúr` — `faktúr` LEN s diakritikou, aby poľské „Faktura" na CMR nefalošne prešlo ako použiteľné; bezpečný smer = skús vision, definuj „použiteľné" ÚZKO; prázdne OCR = 0 signálov = nepoužiteľné → zachovaný starý needs_vision prípad). `_notify` pre skener → `delivery_notes_channel_id` (243), nový alert kind `scanner_not_dl` (v `dl_alerts.GROUPED_ITEM_KINDS`, hlavička „naskenujte znova stranu s položkami") nesúci rozpoznaný typ dokladu z rozšíreného vision poľa `doc_type` („vyzerá ako CMR"); ne-skener nezmenený (#308/#310 ops). Verdikt sa počíta RAZ v `sweep` (`_classify`) a ide do `_apply_rescue` aj `_notify`; dedup kľúč = kind daného odosielateľa (`scanner_not_dl` vs `human_processing_review`), #390 SQL guardy `kind IN (both)`. RED (`a1a37fc`, test_human_processing scanner/CMR/dedupe) → GREEN (`44ef035`). Fresh-context adversárny review (7 lenses): 1 Medium correctness nález OPRAVENÝ in-lane (`5e75fd5`) — `_QTY_UNIT_RE` počítal `kg` váhu ako položku, takže reálny CMR (brutto/netto kg + 13-miestne číslo zásielky) by prešiel ako „použiteľný" a preskočil vision = presne #436 bug; „použiteľné" teraz = ≥2 RÔZNE EAN-13 ALEBO SK kľúčové slovo (qty+unit/kg zahodené). Zvyšok 7 lenses PASS. Lokálne: test_human_processing 21 passed, ruff clean (whole tree), mypy clean pre zmenené moduly (5 pre-existujúcich import-not-found v extract.py/dl_extract.py = lokálne chýbajúce natívne deps, nie môj kód). Playbook: `.claude/rules/human-processing-sweep.md` (#436 sekcia — gate „OCR unusable" nie needs_vision + scanner→243 carve-out).
- **#442 (0.9.159) — board redesign lane 1: kostra jednotnej nástenky `/nastenka` + audit_log + mäkké mazanie.** Prvá lane epicu #441 (spec `docs/superpowers/specs/2026-09-15-board-redesign-design.md`, prístup A: Flask + Jinja v súboroch + vanilla JS ES moduly, bez build kroku/CDN). Nový `app/board/` blueprint: `register_board(app,deps)` (`/nastenka` + `/nastenka/<tab>` + `/api/board/ping`), `auth.py` = JEDEN gate pre `/nastenka*`+`/api/board/*` (rola sklad z OBOCH HMAC kľúčov — `SKLAD_ROLE` aj `SKLAD_DL_ROLE`; admin zo `session.auth`; inak `/login`/401 — deleguje sa z `httpapi._gate`), `services/audit.py` (leaf: `record()` + `restore()` skeleton, importovaný lazy z `orders.teach` bez cyklu). `templates/board/layout.html` (hlavička `[data-testid=version]` + 11 záložiek §4 ako „Pripravujeme"; admin navyše „Maily"→`/`) + `static/board/{board.css,api.js,ui.js}` (shippujú cez existujúce `COPY app/ ./app/`, žiadna zmena Dockerfile). Migrácie r15 `audit_log` (§5) + r16 `deleted_at` na 9 tabuľkách (`retired` riadky backfillnuté a zjednotené; čítače snapshotov/pamätí treat `deleted_at IS NOT NULL` ako `retired`). Existujúce `/znalosti` DELETE endpointy (6) už MÄKKO mažú (`deleted_at`+`retired`) + zapisujú `audit_log`; `teach.answer`/`undo` tenký best-effort audit hook. `/sklad/<k>` aj `/sklad-dl/<k>` → `/nastenka` (oba kľúče platné, staré stránky bežia ďalej do lane 8). RED (`5bd6353` test_board + rozšírený characterization/httpapi/e2e/conftest) → GREEN (`9a23e3e`) → review-fix (`2496c30`: `deleted_at IS NULL` filter aj do dl_item/dl_supplier/mail_rules readerov, reálne id v customer/dl-supplier audite). Adversárny fresh-context review (10 lenses): 0🔴, 2🟡 opravené in-lane, 2🔵 zdôvodnené/deferred. Gates: ruff+mypy clean (88 files), pytest 2088 passed / coverage 93.76 %, orders+DL eval korpusy exit 0. PR #450 merged (`36ba882`), nasadené 0.9.159 (/health v0.9.159, `state: started`). Živé overenie: `/sklad/<key>` → 302 → `/nastenka` → DOM v0.9.159 (= /version), 11 záložiek, 0 console chýb; staré `/otazky` stále 200. Playbook: `.claude/rules/board.md` (nový) + router v CLAUDE.md.
- **#443 (0.9.160) — board redesign lane 2: záložky Otázky sklad + Otázky objednávky.** Druhá lane epicu #441. Dve otázkové záložky ako TENKÝ pohľad nad existujúcou `teach`/`hold`/`dl_questions` mašinériou — delegované, NIE duplikované. Kľúčový refaktor: `httpapi_orders_questions.register()` teraz VRACIA `{"answer","undo"}` dispatch callables (jeho dve bývalé route-telá zdvihnuté na nested fns s param `allowed_kinds=_UNSET`); legacy `/api/orders/question/<qid>/answer|undo` sú tenké wrappery s byte-identickým správaním (`allowed_kinds` sa odvodí zo session role keď `_UNSET`); `create_app` odovzdá dispatch do `register_board(...,questions_api=)`. Nové `app/board/`: `services/questions.py` (scope-filter+status-bucket+search list nad `teach.open/expired/recently_taught`; `reopen` expired→open atomický `WHERE status='expired' RETURNING` guard + `hold.reopen_expired` + audit `reopen`; scope-guarded náhľad originálu — `/api/board/files`,`/eml` len pre message NESÚCI otázku, BEZ rozšírenia legacy token-only `/files` gate), `questions_orders.py` (route vrstva + ORDERS card-affordances), `questions_dl.py` (DL card-affordances). Board answer/undo delegujú s `allowed_kinds=None` (§6: záložka rozhoduje o scope, nie kľúč — DL session smie odpovedať ORDERS otázku, legacy endpoint ju stále odmieta). Nové `teach.expired_questions()` sibling; `hold.reopen_expired()` facade re-export (flip held_orders `release_reason='expired'`→held, message NEresetuje). `templates/board/{questions.html,_placeholder.html}` + `layout.html` content_template/tab_script wiring; `static/board/tab-questions.js` + board.css. Mapovanie: `otazky-objednavky`=orders kindy, `otazky-sklad`=dl kindy (potvrdené z E2E zadania). BEZ migrácie (order_questions.status + audit_log už z lane-1). RED (`d0a88f0` test_board_questions + rozšírený e2e/characterization) → GREEN (`cd4efa4`) → review-fix (`ab7d6a4`). Fresh-context adversárny review (7 lenses): všetkých 7 PASS, 0 blokujúcich; 2 low-severity opravené in-lane (refresh-freeze pri opustenom focus → `editingOpen()` kontroluje aktívny focus/obsah; search „e-mail" → `_matches` zahŕňa context.sender_email), 1 zdôvodnene ponechaný (client-side op→body kontrakt medzi starým+novým board JS, server nedupl., zjednotí lane 8). Gates: full suite green (0 fail/skip), ruff+mypy clean. Playbook: `.claude/rules/board.md` (lane-2 sekcia — delegate-never-duplicate, scope-by-tab, reopen mechanika, scoped preview).
- **#444 (0.9.161→deploy; test doplnok 0.9.162) — board redesign lane 3: záložka Kôš / História zmien (vrátiť čokoľvek).** Tretia lane epicu #441 (spec §4/§5, lane 3 v §8). Nad `audit_log` (r15) + `deleted_at` (r16) z lane 1: nový `board/trash.py` (API `/api/board/audit` zoznam s filtrom tabuľka/akcia-skupina Zmazané·Zmeny·Odpovede + fulltext + stránkovanie; `/api/board/audit/<id>/restore` = Vrátiť), plný `restore()` v `board/services/audit.py` vetviaci podľa akcie: delete→un-delete+`retired=false`+rebuild snapshotu; create→mäkko zmazať; update→zapísať `before` (stĺpce validované cez information_schema, jsonb cez Json, idempotency 409); answer→`teach.undo`; undo→znovu poslednú `answer` (scoped `id<audit_id`, drží qty/price); reopen→re-expire. Každé vrátenie = nový append-only `restore` riadok; dvojité→409; `restore` riadok nevrátiteľný; NIKDY neposiela do ORIONu ani nesiaha na `edi_sent`/`desadv_sent`. Záložka `kos` cez lane-2 `_TAB_CONTENT` mechanizmus (`board/trash.html` + `static/board/tab-trash.js` — tabuľka kedy/kto/čo/akcia/zhrnutie/odkaz, filter chips, debounce hľadanie, pager, Vrátiť s confirm). RED `d05d772` → GREEN `5b8a73a` → review-fix `711bc08`. Merge lane 2 (0.9.160) medzitým: merge origin/dev, verzia vyriešená na 0.9.161, konflikty (board `__init__.py`/layout/css/md, characterization, verzia) vyriešené prispôsobením na lane-2 `_TAB_CONTENT`/`_render` architektúru. Dva nezávislé fresh-context adversárne review passy: 0 🔴 (1🟡+🔵 nálezy zafixované in-lane — update idempotency guard, undo id-scoping, schema-scope, drop unused param). Gates: ruff+mypy clean, full suite (čerstvý kontajner) exit 0 / 0 fail. PR #452 merged (`47a837c`), nasadené 0.9.161 (/health v0.9.161, state started). Živé overenie: clean session `/`→`/login`, `/sklad/<key>`→`/nastenka`, `/nastenka/kos` render (chips+tabuľka+tab-trash.js), `/api/board/audit` 200 (prázdny — na PROD zatiaľ žiadne board zmeny), DOM v0.9.161=/version, 0 console chýb. Doplnok (0.9.162, follow-up PR): chýbajúci real-browser Playwright E2E `test_board_kos_delete_a_card_then_restore_it_in_the_browser` (admin zmaže kartu cez /znalosti API → v Koši → Vrátiť → späť v /api/znalosti/catalog, 0 console chýb) — lane 1/2 ho mali, lane 3 dobehla. Playbook: `.claude/rules/board.md` (sekcia restore-per-action).
- **#445 (0.9.162→merge 0.9.163) — board redesign lane 4: záložky Produkty sklad + Produkty objednávky.** Štvrtá lane epicu #441 (spec §4/§5, lane 4 v §8). Dve produktové záložky ako TENKÝ pohľad nad existujúcimi katalógovými strojmi — delegované, NIE duplikované (`board.md`: „call the existing engines, never copy them"). Nové `app/board/`: `services/catalog.py` (list+search+paging, card detail, upsert, delete — scope→delegát dispatch `_SCOPES`), `services/catalog_aliases.py` (alias list/add/remove + search-by-wording, split aby oba moduly ostali ≤200 r.), `products_orders.py` (route vrstva, všetky `/api/board/products*` na JEDINOM board blueprinte, `?scope=orders|dl`), `products_dl.py` (DL card FIELD deskriptor doplnok/mass/sklad/cena — dátový modul bez rout, ako `questions_dl.py`). Sklad rola dosiahne OBE scope cez board gate (DL produkty boli predtým len admin) — rozhoduje ZÁLOŽKA nie kľúč (§6). Create/update/delete volajú presne tie `snapshot`/`dl_snapshot` funkcie čo `/znalosti` (`upsert_catalog_card` tri-state alias / `upsert_dl_catalog_card` + rebuild; `retire_*` už #442 nastavuje `retired`+`deleted_at` → mäkké mazanie by construction) + `audit.record` na každú zmenu (§5). Aliasy karty = wording→gtin pamäť: orders global+per-customer (`memory`), DL per-supplier (`dl_memory`) — pridané nové `dl_memory.add_dl_alias` + `delete_dl_item_memory_row` (source='human', mäkké mazanie, supplier_ean+curated scoped — paralely `memory.py` helperov, keďže `teach._undo_dl_item` tvrdo DELETE-uje). `templates/board/products.html` + `static/board/tab-products.js` (hľadanie debounce, stránkovanie, inline editor, per-card alias manager, soft-delete confirm→toast „Vrátiť v Koši", refresh-safety) + board.css produkty. `_TAB_CONTENT`/`register_board` doplnené o produktové riadky. BEZ migrácie (audit_log + deleted_at + dl_item_memory.deleted_at už z lane 1). RED (`a3cd002` test_board_products 20 fail + 2 e2e Playwright) → GREEN (`4782770`) → merge origin/main lane-3 (`0d3d5d9`, verzia 0.9.162→0.9.163, aditívne konflikty _TAB_CONTENT/register/EXPECTED_ROUTES/e2e vyriešené keep-both) → review-fix (test_update_dl_card_name_only_keeps_mass_sklad_cena + DL delete frozen-snapshot parita + comment). Fresh-context adversárny review (7 lenses + štruktúra): **0 🔴 1 🟡 4 🔵** — 🟡 (chýbajúci DL name-only tri-state test, cesta bola správna) + 2🔵 opravené in-lane, zvyšné 🔵 zdôvodnene ponechané. Gates: ruff+mypy clean (96 files), full suite exit 0, test_board_products 25/25. Playbook: `.claude/rules/board.md` (lane-4 sekcia).
- **#447 (0.9.165, worktree-return) — board redesign lane 6: záložky Naučené sklad + Naučené objednávky.** Šiesta lane epicu #441 (spec §4/§5, lane 6 v §8). Jeden ZJEDNOTENÝ „rule" riadkový model nad 5 „naučenými" tabuľkami — TENKÝ pohľad, delegované, NIE duplikované. Scope→kind: `naucene-objednavky`=orders (`mail`/`mail_rules` · `alias`/`item_memory` curated · `global`/`global_item_memory`), `naucene-sklad`=dl (`dl_alias`/`dl_item_memory` curated · `supplier`/`dl_supplier_memory`). Nové `app/board/`: `services/rules.py` (unified row model + list/search/paging + origin joiny; `scope_of`/`table_for`/`SCOPE_KINDS` = single source, trusted-literal table map), `services/rules_edit.py` (update+delete dispatch, každý deleguje na engine write path; `UniqueViolation`→`RuleCollision`→409), `rules_orders.py` (route vrstva `/api/board/rules*` `?scope=&kind=` + ORDERS editor deskriptor), `rules_dl.py` (DL editor deskriptor, dátový modul). Mäkké mazanie: aliasy reuse lane-4 soft-delete helperov; `mail_rules`+`dl_supplier_memory` nemali soft-delete (teach `_undo_mail`/`dl_supplier_memory.forget` tvrdo DELETE-ujú pre reopen-otázku undo) → pridané kanonické `teach.soft_delete_mail_rule` + `dl_supplier_memory.soft_delete`; readery už filtrujú `deleted_at IS NULL` → mäkké mazanie zastaví matching by construction (RED→GREEN dôkaz). Update in-place + delegovaný: nové `teach.update_mail_rule` (normalizuje subject_key cez `subject_key()`, inak matching prestane fungovať), `memory.update_global_row`/`update_item_memory_row`, `dl_memory.update_dl_item_memory_row` (prepočítavajú `item_key`), `dl_supplier_memory.update_by_id`; každý vracia `before` reálnych stĺpcov → audit `update` → lane-3 Kôš restore. ŽIADNA create cesta (pravidlá vznikajú len odpoveďou na otázku) → #407 scanner-guard neobíditeľný z tejto záložky (regresný pin). Origin link → `/nastenka/otazky-<sklad|objednavky>?q=<message_id>&status=answered` (tab-questions.js seedne filter z location.search, aditívne) + inline „Originál" cez existujúci scope-guarded `/api/board/questions/<qid>/preview`. `templates/board/rules.html` + `static/board/tab-rules.js` (kind chips, hľadanie debounce, stránkovanie, inline editor z `meta.edit`, soft-delete confirm→toast „Vrátiť v Koši", refresh-safety) + board.css. BEZ migrácie (audit_log `_SOFT_DELETE_TABLES` už pozná všetkých 5, conftest TRUNCATE tiež). RED (`6de4bd0` test_board_rules 19) → GREEN (engine helpery + board vrstva) → review-fix (3 reálne bugy: 🔴 origin link zlý tab slug `otazky-<scope>`→`otazky-<sklad|objednavky>`; 🟡 update_mail_rule neukladal normalizovaný subject_key; 🟡 unique-collision→409 nie 500; +2🔵 audit after item_key + status whitelist; +2 nové regresné testy → 21). Fresh-context adversárny review (7 lenses): 1🔴 2🟡 3🔵, všetky opravené in-lane. Lokálne: ruff clean, mypy clean (8 zmenených modulov), test_board_rules 21/21, plná suite (venv). Worktree-return: supervisor integruje (PR/CI/merge/deploy). Playbook: `.claude/rules/board.md` (lane-6 sekcia).
- **#448 (0.9.166, PR #457 merged 79ee245, deployed + live-verified) — board redesign lane 7: záložky História objednávok + História dodacích listov.** Siedma lane epicu #441 (spec §4/§5, lane 7 v §8). Dve záložky histórie ako TENKÝ pohľad nad `order_runs`/`order_items`/`email_events` + `edi_sent`/`desadv_sent` ledgermi — čítanie, nič sa nepárováva nanovo. Nové `app/board/`: `history_orders.py` (route vrstva, `?scope=orders|dl`, list/detail/rerun/manual/teach + scope-guarded náhľad originálu), `history_dl.py` (DL label deskriptor), `services/history.py` (list+filter+search+paging + mapovanie proc_status→plain SK „odišlo do ORIONu·čaká na sklad·na kontrole·zlyhalo·zadané ručne·ignorované"; `orders_edi_names()` + `partner_and_docs()` + `is_history_document()` guard), `services/history_detail.py` (položky+trace z order_items, email_events timeline, náhľad), `services/history_actions.py` (rerun = sankcionovaný reset `attempts=0` LEN keď doklad nič nenahral: not-busy + proc_status=error + žiadny uploaded ledger + živý ORION presence, fail-safe refuse pri nedostupnom ORIONe; manual = `hold._resolve_one_manually` release bez odoslania, message-scoped, orders-only), `services/teachback.py` (zapíše `item_memory`/`dl_item_memory` `source='teachback'` + audit `teach`, nikdy nemení odoslaný doklad/ledger/ORION). `resolve()` taught-rung rozšírený `source='human'`→`source IN ('human','teachback')` v `memory.py`+`dl_memory.py` (corpus-neutral) + `'teachback'` do oboch `CURATED_SOURCES`; `dl_memory.add_dl_alias` dostal `source=` param. `audit.py` nové restore vetvy `teach`(soft-delete) / `rerun`+`manual`(400 nezvratné). `templates/board/history.html` + `static/board/tab-history.js` (list, status chips, date-range, detail drawer s položkami+trace+timeline+originál, per-riadok „✏️ Doučiť" card-picker cez `/api/board/products`, akcie rerun/manual s allowed/reason) + scoped `.board-history`/`.h-*` CSS. `_TAB_CONTENT` + EXPECTED_ROUTES (7 rout) doplnené. BEZ migrácie. RED (`5324b48` test_board_history 25) → GREEN (`8b21ca4`) → review-fix (`3575ea2`) → merge origin/main lane-6 (`8507aba`, 0.9.165→0.9.166, aditívne konflikty _TAB_CONTENT/EXPECTED_ROUTES/board.css/verzia keep-both). Fresh-context adversárny review (7 lenses): **1 🔴 opravený in-lane** — orders ORION-guard kľúčoval len na `messages.edi_file` (nastavené len pri POTVRDENOM uploade); zlyhaný upload s NULL edi_file ale bajtami na ORIONe by sa mohol re-shipnúť → fix `orders_edi_names()` číta aj `result.order_results[*].edi_filename` (nastavené pri BUILD, pred uploadom) + RED regresný test. Gates: full suite green (93% coverage), ruff+mypy clean, e2e-orders+e2e-dl korpusy green. PR #457 merged, nasadené 0.9.166 (/health v0.9.166, state started). Živé overenie (read-only): clean session `/`→`/login`, `/sklad/<key>`→`/nastenka`, obe história záložky renderujú REÁLNE produkčné doklady (25 objednávok + 25 DL so správnymi SK stavmi a číslami dokladov), detail odoslanej objednávky = 12 položiek + párovanie („Rožok 70g"→karta „Rožok kváskový 70g", pravidlo alias_exact, istota 100%) + timeline, tlačidlo „Spustiť znova" SPRÁVNE DISABLED pre odoslanú objednávku, DOM v0.9.166=/version, 0 console chýb na oboch záložkách. Playbook: `.claude/rules/board.md` (lane-7 sekcia + CSS-scoping + edi_file-not-proof-of-upload safety lesson).
- **#459 (0.9.167) — Odoo správy vedú rovno na správnu záložku novej nástenky (kľúč→záložka, ?next hlboký odkaz).** Owner rozhodol nepredstavovať novú nástenku skladu; stačí, aby odkazy v Odoo správach viedli rovno na správnu záložku (objednávkové→`/nastenka/otazky-objednavky`, dodacie→`/nastenka/otazky-sklad`). Nový LEAF `app/board/links.py::board_link(cfg, kind, question_id=None)` stavia `<base>/sklad[-dl]/<key>?next=/nastenka/<slug>[%3Fq%3D<id>]`; kľúč+base z `linkutil` (NIKDY `public_base_url` — 0.9.10 bug). Dva funnel helpery `report.sklad_link`/`dl_sklad_link` preroutované naň (LAZY import, žiadny cyklus) → VŠETCI warehouse-link generátori (pipeline/hold_close/hold_redecide/question_alerts/dl_message/dl_document/dl_report) dostali nový tvar bez zmeny call-site; text „📋 Rieš na nástenke" nezmenený. `dashboard_link` operátorské alerty („Otvor dashboard" na `/` v dl_alerts/human_processing) zámerne NEZMENENÉ — nie sú warehouse odkazy. Kľúčové routy `/sklad/<k>`,`/sklad-dl/<k>` teraz honorujú `?next` cez `links.safe_next` (len interné `/nastenka…`, open-redirect guard) a bez `next` presmerujú podľa druhu kľúča; HMAC kontrola nezmenená. `tab-questions.js`: číselné `?q=<id>` = deep-link scroll+highlight `.q-card--focus` na `#q-card-<id>` (ne-číselné `q` ostáva #447 search-seed). RED (`5d26cd4` test_board_links 30 + deep-link Playwright + e2e wait_for_url→regex) → GREEN (`board/links.py`, report preroute, route ?next, JS+CSS). Gates: ruff+mypy clean, plná suite green. Playbook: `.claude/rules/board.md` (#459 sekcia — funnel-preroute, safe_next open-redirect, ?q/#447 kolízia).

- **#449 (0.9.168) — board redesign lane 8: vypnutie starých stránok + zmazanie šablón/hash-testov (epic #441 KONIEC).** Ôsma a POSLEDNÁ lane epicu #441 (spec §6/§8). Owner ROZHODNUTÉ (2026-09-16 na #449): spustiť TERAZ, staré presmerovať, šablóny+hash-testy zmazať, #441 zavrieť, návod skladu NEposielať. Staré stránky `/otazky`, `/otazky-dl`, `/znalosti`, `/znalosti/<ean>` ostávajú zaregistrované ako routy, ale telo je iba `redirect()` → `/nastenka/otazky-objednavky` (orders), `/nastenka/otazky-sklad` (dl), `/nastenka/produkty-objednavky` (znalosti), `/nastenka/zakaznici?q=<ean>` (per-zákazník). Presmerovanie platí pre KAŽDÚ rolu — `_gate` má tie 4 cesty v OPEN-bypasse (anon dostane 302, board si prihlásenie vynúti na cieli); kľúčové cookies `/sklad`,`/sklad-dl` ostávajú platné; rolový fallback v `_gate` (bounce sklad zo `/`) teraz vedie na `board_links.ORDERS_TAB`/`DL_TAB`. Zmazané `ASK_HTML`/`ASK_DL_HTML`/`ZNALOSTI_HTML`+`_ASK_HTML_TEMPLATE` z `httpapi_templates.py` (~1004 riadkov; ostávajú len `LOGIN_HTML`+`DASH_HTML`, ich literály+sha piny NEDOTKNUTÉ), ich sha256 piny + `_TEMPLATE_CONSTANTS` v characterization teste odstránené. `httpapi_security.py`: zahodený LEN page-only `SKLAD_ZNALOSTI_PAGE` + stránkové položky (`/otazky` z `SKLAD_PATHS`, `/otazky-dl` z `SKLAD_DL_PATHS`); API allowlisty (`/api/orders/*`, `SKLAD_ACTION`, `SKLAD_ZNALOSTI_API`, `SKLAD_DL_ZNALOSTI_API`) OSTÁVAJÚ (board na tie stroje deleguje in-process; `/api/znalosti/*`+`/api/orders/question/*` musia zostať dostupné pre sklad rolu — dôkaz `test_znalosti_api_still_reachable_via_the_warehouse_link`). Admin `/` dashboard OSTÁVA; `__SKLADLINK__`/`__DLSKLADLINK__` teraz nesú `?next=<board tab>` (tvar #459 board_link, host-base operátora). `httpapi_znalosti.py`: `znalosti_page` → redirect, zmazané importy `ZNALOSTI_HTML`/`session`/`__version__`, pridané `redirect`/`quote`; API endpointy nezmenené. RED (`3174b9c` test_httpapi + characterization redirect testy per rola, incl. no-cookie→login) → GREEN (routes+gate+security+templates+znalosti + aktualizácia starých 200-asserting testov na redirect: test_httpapi.py 6 testov, test_board.py, test_httpapi_new_dl.py, test_httpapi_znalosti.py 3 testy; zmazaných 17 starých-stránkových Playwright testov z test_e2e.py — funkčnosť pokrývajú board tab testy — + aktualizovaný skeleton test). Route-map char-test NEZMENENÝ (routy stále existujú ako redirecty), pridaný `test_the_retired_warehouse_pages_are_redirects_not_html`. Gates: ruff+mypy clean, plná suita green (coverage ≥85%), e2e-orders+e2e-dl korpusy green. Zatvára #449 aj #441 (epic KONIEC, 0.9.159→0.9.168). Playbook: `.claude/rules/board.md` (lane-8 sekcia — redirect-not-deregister, open-bypass per rola, API allowlist ostáva).

- **#462 (0.9.169, PR #463 merged 788036b, deployed + live-verified) — DL: kg-sledovaná karta s prázdnou mass + kusy → tichý ×N (HOLD + otázka na hmotnosť).** Živý incident: LESAFFRE droždie (karta „Droždie" (reálny gtin mimo git — verejné repo), doplnok „Rekord 10 kg, drevo", `sklad=100`, `mass=NULL`) odišla do ORIONu 7 kg namiesto 70 — `dl_match._mass_kg` pri prázdnej mass slepo padla na `mass_grams(<znenie DL „Rekord 1 kg, drevo">)/1000 = 1.0` (váha BLOKU, nie 10 kg kartónu), `desadv_edi.generate()` R84 `elif mass>0` poslal 7×1.0. Cena 0/1.0 → R85 dosadila €/kg, takže money gate nechytil. Trieda: 244/252 kg-sledovaných kariet má prázdnu mass. **Kód (všeobecná trieda):** `_mass_kg` vracia `float | None` — pri kg-sledovanej karte s prázdnou mass dôveruje hodnote LEN keď váha zo znenia AJ z doplnku karty obe parsujú a ZHODUJÚ sa (`WEIGHT_TOLERANCE`); inak `None`; ne-kg karty a „vajcia" (eggs, mirror generate `is_kg_tracked`) ostávajú historický float. `Decision.mass: float|None`. `dl_document._process_document` (LIVE only) DRŽÍ takýto doklad (#365 vzor: žiadny claim/upload, ❗ `build_review`) a vytvorí nový `dl_mass` board otázku „koľko kg má 1 kus/kartón"; odpoveď zapíše `mass` na KARTU (override cez nová `dl_snapshot.set_dl_card_mass`, naučí sa raz) a `release_for_question` prepustí celý doklad (7 KS → 70.000 kg). Pokrýva aj #366 `else` rung (neparsovateľná mass → hold, nie neprepočítané). Nový `dl_mass` `teach.KINDS` druh (present/validate/apply/undo, kľúč `dlmass:{gtin}`); zapojené: `httpapi_security.DL_KINDS`, generic answer dispatch, `dl_questions` still_open/close/sibling brány, `question_alerts._DL_KINDS`+`_WHAT` (reminder na DL kanál), `board/questions_dl.DL_CARD_ACTIONS` („Neviem" defer), `tab-questions.js` (číselné pole). `generate()` NEZMENENÝ (byte-parita `desadv_reference.json` + e2e-dl korpus). Commity: bump `11110f2`, RED `b5fcdcb`, GREEN `37d327b`, review-fix `89dd768` (supplierName→name), review2 `b7e9ee7` (vajcia + reminder routing + defer + docstring). Fresh-context adversárny review (7 lenses): 0🔴 4🟡 3🔵 — 🟡-1 supplierName, 🟡-2 vajcia, 🟡-3 reminder routing, 🟡-4 false byte-safe komentár, všetky opravené in-lane; 🔵-5 non-warehouse close už pokryté (close gates majú dl_mass), 🔵-6 defer pridaný, 🔵-7 dead template preskočené. Gates: ruff+mypy clean, plná suita green (coverage 93.13%), DL+orders korpusy green offline. CI: 1 flaky Playwright timeout (`test_board_products_orders_tab_search_edit_delete`, products tab — nedotknutá; prešlo na PR run identického obsahu) → 1 rerun green. Nasadené 0.9.169 (/health + /version + DOM v0.9.169, state started, Otázky sklad tab 0 console chýb, served tab-questions.js má q-massin). Dáta: karta Droždie dostala `mass=10` cez `POST /api/board/products?scope=dl` (app path, NIE SQL), read-back `dl_catalog_overrides` mass=10 (ostatné polia zachované). Incident DESADV už `imported` → NIKDY re-ship (#239). Playbook: `.claude/rules/n8n-workflow-edits.md` (#462 sekcia — kg-tracked blank-mass hold, 8-miestny nový-kind checklist).
