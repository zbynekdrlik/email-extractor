...
- párovanie ZÁKAZNÍKA (`customer.resolve`) — viac-prevádzková rodina kariet, kmeň názvu (`_name_stem`), adresa doručenia NAD llm pickom, korpus-safety brána `owners==0`, anti-pattern doména (#435) → `.claude/rules/customer-resolution.md` (auto-loads on `app/orders/customer.py`)
- zdroj kariet katalógu = LEN Postgres, Google tabuľka „EAN slovnormal" je ZRUŠENÁ (#383 — nikdy ju nečítaj); pridanie karty cez nástenku (záložka Produkty; #449 lane 8 — stará stránka `/znalosti` vypnutá, presmerúva na nástenku; API `/api/znalosti/*` ostáva), alias/`doplnok` tri-state override, reopen expirovanej (#341) otázky → `.claude/rules/catalog-sources.md` (auto-loads on `app/orders/snapshot.py`, `app/httpapi_znalosti.py`, `app/orders/teach.py`)
...
- vyčistenie ops backlogu (`human_processing` catch-all → reclassify na `no_processing`, session cookie cez /login, reverzibilné, nič sa nepustí) (#336/#338) → `.claude/rules/ops-backlog.md` (auto-loads on `app/orders/human_processing.py`, `app/httpapi*.py`)
- CODEX dôkaz o objednávkach (codex-bridge DuckDB read-only join/dedup, NICO→AEDIEAN most, neutrálne zavretie `mail` otázky bez `mail_rules`, push tool DI seam) (#342) → `.claude/rules/codex-orders.md` (auto-loads on `app/orders/codex_orders.py`, `tools/codex_orders_push.py`, `app/httpapi_codex.py`)
- úprava množstva/ceny riadku na nástenke tak aby POTVRDENÁ hodnota odišla (multi-question hold: persist na otázku + read-back všetkých pri shipe; `_num` odmieta ≤0; cena sa do ORIONu neposiela; zmena `ORDER_SCHEMA` = re-record len extraction cache) (#360) → `.claude/rules/board-line-edit.md` (auto-loads on `app/orders/hold.py`, `app/orders/teach.py`, `app/httpapi_orders_questions.py`, `app/httpapi_templates.py`)
- AI bezpečné zahodenie mailu, ktorý nie je objednávka ani dodací list (samostatný `mail_kind` klasifikátor na `orders==[]` vetve, brána = veľké AND s fail-safe „pýtaj sa", diakritické fold-first vetá, dry-run prepínač `ai_not_order_discard`, restore endpoint + „Zahodené AI" tab, `_discard_no_processing` obchádza `_finish`, ScriptedClient obsluhuje klasifikátor podľa `name`) (#376) → `.claude/rules/mail-kind.md` (auto-loads on `app/orders/mail_kind.py`, `app/orders/pipeline.py`, `app/httpapi_dashboard_data.py`)
- dlho-žijúca worker slučka drží vlastné spojenie → MUSÍ reconnectnúť na `psycopg.OperationalError` (Postgres spadne+zotaví sa, spojenie ostane zatvorené; incident #380 — 2h46m nulového spracovania) → `.claude/rules/worker-connection-lifecycle.md` (auto-loads on `app/main.py`, `app/orders/worker.py`)
- grounding dátumu dodania + DRŽANIE objednávky pri preklepe/vymyslenom dátume (napísaný deň nesedí s extrahovaným, napr. „14.8." + „pondelok" → 14.09.); #163 to zahadzovalo, #420 drží + `date` otázka s kandidátmi → `.claude/rules/date-grounding.md` (auto-loads on `app/orders/extract.py`, `pipeline.py`)
- tichá strata objednávky: `hold.place` NIKDY nedostane `matched=None` (drž na placeholder `Matched(unmatched)`, nikdy negatuj placeholder na `is_change` — otázku áno, placeholder nie); pád pipeline na finálnom pokuse MUSÍ pingnúť ops (`pending_alerts`, oba engine); reset spadnutej správy vyžaduje `attempts=0` (#431) → `.claude/rules/hold-crash-safety.md` (auto-loads on `app/orders/hold.py`, `pipeline.py`, `worker.py`, `static_worker.py`)
- jednotná nástenka `/nastenka` (blueprint `board`, jeden auth gate pre oba kľúče + admin, šablóny+JS v súboroch, `deleted_at` mäkké mazanie + `audit_log`, routes/services split; #449 lane 8 — staré stránky `/otazky`/`/otazky-dl`/`/znalosti` VYPNUTÉ, presmerúvajú na nástenku) → `.claude/rules/board.md` (auto-loads on `app/board/**`, `templates/board/**`, `static/board/**`)
