...
- jednotná nástenka `/nastenka` (blueprint `board`, jeden auth gate pre oba kľúče + admin, šablóny+JS v súboroch, `deleted_at` mäkké mazanie + `audit_log`, routes/services split; beží popri starých stránkach do lane 8) → `.claude/rules/board.md` (auto-loads on `app/board/**`, `templates/board/**`, `static/board/**`)
