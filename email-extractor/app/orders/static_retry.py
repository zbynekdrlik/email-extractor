...
2. **The document's stable identity on ORION is the WHOLE static filename.** Unlike the
   ...
   archCodex rename via the SHARED `desadv_edi.matches_wire_prefix` (never a second copy of
   the prefix logic). And because the static name does NOT encode content or delivery date,
   a same-order-number correction can produce the SAME filename with different content —
   `_confirmed_collision` refuses to trust a presence match when a DIFFERENT, already-
   confirmed `edi_sent` row occupies that name, so an ambiguous match falls to alert+release
   instead of silently confirming (and dropping) our order. That is the static analogue of
   `desadv.has_confirmed_collision`, whose own collision source (a truncated stable prefix)
   does not apply here — the static identity is never truncated.
"""
...
def _present_on_orion(dirs: dict, filename: str) -> bool:
    for folder in _PRESENCE_FOLDERS:
        for name in (dirs or {}).get(folder) or ():
            if desadv_edi.matches_wire_prefix(name, filename):
                return True
    return False


def _confirmed_collision(conn, filename: str, content: str) -> bool:
    """Is a DIFFERENT, already-CONFIRMED `edi_sent` row occupying this exact filename? The
    static filename encodes partner/order/store, NOT content or delivery date, so a same-
    order-number correction can share it with a different document. When that has already
    shipped, a bare presence match on the name cannot prove OUR (different-content) bytes
    landed — confirming would silently DROP this order (the #239 mirror: silent LOSS)."""
    chash = edi.content_hash(content)
    row = conn.execute(
        "SELECT 1 FROM edi_sent WHERE filename = %s AND uploaded_at IS NOT NULL "
        "AND content_sha256 <> %s LIMIT 1", (str(filename or ""), chash)).fetchone()
...
def check_landed(conn, cfg, list_dirs, filename: str, content: str) -> bool | None:
    ...
    - `None`  — the check itself could not be attempted (the SFTP connection that just
      ...
      match was found but is NOT trustworthy (`_confirmed_collision`). Either way NO retry
    ...
    try:
        ...
        collision = landed and _confirmed_collision(conn, filename, content)
    except Exception:
