...
def register(app: Flask, deps: Deps) -> None:
    ...
    def api_znalosti_dl_suppliers_upsert():
        ...
        city = str(body.get("city") or "").strip()
        invoice_is_dn = bool(body.get("invoice_is_delivery_note", False))
        try:
            with deps.db() as c:
                ...
                # #406: set the invoice_is_delivery_note flag after the upsert — a
                # separate UPDATE is safe (autocommit) and avoids threading a new param
                # through the complex upsert_dl_supplier signature with its advisory locks.
                c.execute(
                ...
                dl_worker.release_for_supplier_card(c, deps.cfg, ean, name, emails)
        except snapshot.DuplicateEan as e:
