"""PostgreSQL baseline schema (DDL) — extracted verbatim from db.py (#309).

This is migrate revision 1 (`db.REVISIONS[0]`, the self-healing baseline). The
statement ORDER is load-bearing: it is applied in sequence and later entries are
in-baseline migrations (ALTER/DROP-INDEX/DO blocks) appended over time by
chronology, NOT grouped by domain — never reorder them. Pure data, no logic.
"""
