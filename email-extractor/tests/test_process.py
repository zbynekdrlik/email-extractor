...
def _build_email(body="Dobrý deň, posielam objednávku.", attachments=()):
    m = EmailMessage()
    m["From"] = "Karmen s.r.o. <orders@karmen.sk>"
    m["To"] = "sklad@slovnormal.sk"
    m["Subject"] = "Objednávka 1030"
    m["Message-ID"] = "<test-123@karmen.sk>"
...
def test_headers_and_identity():
    ...
    assert rec["identity"] == "<test-123@karmen.sk>"
    assert rec["headers"]["from_addr"] == "orders@karmen.sk"
    assert rec["headers"]["to_addrs"] == ["sklad@slovnormal.sk"]
